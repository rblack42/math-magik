...
..  glossary::
    ...
    Given When Then
        A version of |TDD| where we define the state of the project (Given) and
        an action we will take (When) and the result we expect (Then).  See
        `given when then`_
    ...
    semantic versioning
        A style if version numbering common in software development. See `semantic versioning`_
