...
set up a |PY| class that will manage access to |OSC|_ from the command
line.

Feature 2: Add |OSC|_ Management class
****************************************

The primary purpose of the |OSC|_ manager class is to isolate running
OpenSCAD to perform needed operations. For our first test of this class, we
will launch |OSC|_ and fetch the version of the program we are using.
...
The code we need to write will assume that |OSC|_ is on the system path and
can be launched with the name :program:`openscad`.

Test |OSC|_ Version
===================

Users will need to install at least |OSC|_ version 2019.05, so we will set up
...
Our next task involves getting |OSC|_ to generate an :term:`STL` file from
